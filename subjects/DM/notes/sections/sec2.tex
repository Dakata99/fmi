...
% Множества
\begin{definition}
	\textbf{Множество} е първично понятие, което не се дефинира. Интуитивно го определяме като колекция от
	различни обекти.
\end{definition}

\begin{note}
	Обикновено, но не винаги, имената на множествата са главни латински букви: \(A, B, ...\).
...
\begin{example}
	\mexpr{\{a, b, c\} = \{a, a, b, b, b, c\} = \{a, b, c, b, a\} = \{c, b, a\}}.
\end{example}
...
\begin{axiom}
	...
	Казваме, че \(Y\) е \textbf{подмножество (надмножество)} на \(X\) и пишем \mexpr{Y \subseteq X (X \supseteq Y)}. \\
...
\begin{note}
	...
	\textbf{същинско подмножество (надмножество)} на \(X\) и пишем \mexpr{Y \subset X (X \supset Y)}, което
...
\begin{example}
	Разграничение между "максимално по включване" (maximal) и "глобално максимално" (maximum) чрез клика в граф:
...
\begin{definition}
	\textbf{Протоелементите} са първични елементи, не са множества. Множествата изграждаме от протоелементи и/или
	други множества. \\
	Множеството от протоелементите, които може да се появяват, се нарича \textbf{опорно множество}. \\
	Множество от множества се нарича \textbf{фамилия}.
\end{definition}

\begin{example}
	\mexpr{A = \{a, b, c\}} е опорно множество, а фамилии над него са \mexpr{X = \{\{a, b\}, \{c\}\}, Y = \{\{a, c\}\}}
\end{example}

\begin{definition}
	Нека \(X\) е непразно множество. \textbf{Покриване} на \(X\) е всяка фамилия \mexpr{Y = \{Y_1, Y_2, ..., Y_k\}}
	такава, че \(k \ge 1\) и:
	\begin{enumerate}
		\item \mexpr{\forall i \in \{1, 2, ..., k\}: Y_i \subseteq X}
		\item \mexpr{\forall i \in \{1, 2, ..., k\}: Y_i \ne \emptyset}
		\item \mexpr{\cup_{i = 1}^{k} Y_i = X}
	\end{enumerate}
	Ако освен това е вярно и \mexpr{\forall i \forall j (1 \le i < j \le k \to Y_i \cap Y_j = \emptyset)} казваме,
	че \(Y\) е \textbf{разбиване} на \(X\).
\end{definition}

\begin{example}
	Нека \mexpr{X = \{x, y, z, t\}}, покриване на \(X\) е \mexpr{\{\{x, y\}, \{y, z, t\}, \{x, z, t\}\}},
