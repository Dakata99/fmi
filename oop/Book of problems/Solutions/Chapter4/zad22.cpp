...
//-- Този код се включва преди клас IntSet
class AbstractIntIterator
{
public:
    // премества на итератора на следващия елемент
    virtual void operator++(int) = 0;

    // намира стойността на текущия елемент
    // резултатът е неопределен, ако итераторът
    // не е позициониран правилно
    virtual int operator*() = 0;

    // връща истина, когато итераторът е
...
//-- Този код се включва преди класа DynArray
class DynArray;

class DynArrayIterator : public AbstractIntIterator
{
public:
    // създава итератор към масива, сочещ началото му
...
public:
    ...
    // (най-малкият зает индекс, по-голям от index) и -1, ако няма такъв
    ...
    //-- Този код се включва в тялото на клас DynArray
...
//-- Този код се включва след клас DynArray
...
{
    // Когато се достигне краят на масива,
    // currentIndex става -1 и при следващото
    // извикване на ++ се започва от начало
...
public:
    ...
    //----- Този код се включва в тялото на клас IntBinOrdTree
    AbstractIntIterator* start()
    { 
        return new TreeIterator(this);
    }

    // Класът TreeIterator трябва да ползва скритите данни на дървото
...
private:
    ...
    // изтриване на корена на дадено поддърво
    // връща номера на новия му корен
...
//-- Този код се включва след клас IntBinOrdTree
...
{ 
    if(stackTop == -1)
        return;
    // премахване на най-горния елемент от
    // стека и заменяне с левия клон
    // на дясното му поддърво (ако има)
...
// Спускане по левия клон на дървото и записване
// на всички негови елементи в стека
