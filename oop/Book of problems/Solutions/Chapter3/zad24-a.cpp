...
{ 
    ...
    // деклариране и въвеждане на стойности
    // на параметрите, определящи равнината
    double a, b, c, d;
    cout << "a, b, c, d: ";
    cin >> a >> b >> c >> d;

    // търсене на най-тежката точка от масива
    // с цвят в диапазона [1, 5] и лежаща в
    // рaвнината ax + by + cz = d
    double MaxWeight = 0.0;
    WeightColPoint3<double,double,double> MaxWeightPoint;
    int num = 0;    // брояч на точки
    for(i = 0; i < n; i++)
    {
        if(Points[i].getCol() >= 1 && Points[i].getCol() <= 5 &&
            a*Points[i].getX() + b*Points[i].getY() + c*Points[i].getZ() == d &&
            Points[i].getWeight() > MaxWeight)
        { 
    ...
    // извеждане на най-тежката точка от масива
    if(num == 0)
    {
        cout << "Не е намерена точка с търсеното свойство\n";
    }
    else
    {
