...
\begin{definition}
	Ако \(R \subseteq A^2\) е произволна релация и \mexpr{A = \{a_1, ..., a_n\}}, то \textbf{верига} в \(R\) 
	е всяка редица \mexpr{a_{i_0}, ..., a_{i_k}}, където \mexpr{i_0, ..., i_k \in \{1, ..., n\}}, ако 
	\mexpr{a_{i_j} \ne a_{i_{j + 1}}} и \mexpr{a_{i_j}Ra_{i_{j + 1}}} за \mexpr{0 \le j \le k - 1}. 
	Ограничение за \(k\) няма, тоест \(k \ge 0\). 
	Тогава един единствен елемент е верига.
	
	Ако \(a_{i_0} = a_{i_k}\) и \(k > 0\), то веригата е \textbf{контур}. Тогава \(k > 0\) налага \(k > 1\). 
	Един единствен елемент не е контур.
\end{definition}

\begin{example}
	Тук \(abcd\) е верига, а \(bcdb\) е контур.
...
\begin{theorem}
	...
	\begin{proof}
		...
		\mexpr{a \not = b: aRb \land bRa} (не е антисиметрична:
		\mexpr{\lnot(aRb \to \lnot bRa) \equiv \lnot(\lnot aRb \vee \lnot bRa) \equiv aRb \land bRa}).
		
		Но тогава \mexpr{a, b, a} е контур \(\implies\) противоречие с това, че \(R\) няма контури.
	\end{proof}
\end{theorem}

\subsection{Влагане на частична наредба в линейна наредба}
...
\begin{additional}
	...
	\textbf{минимален} в \(R\), ако 
	\begin{center}
		\mexpr{\lnot \exists b \in A, b \not = a : bRa \equiv \forall b \in A, b \not = a: \lnot bRa}.
	\end{center}
	
	Аналогично, \(a\) е \textbf{максимален} в \(R\), ако 
	\begin{center}
		\mexpr{\lnot \exists b \in A, b \not = a : aRb \equiv \forall b \in A, b \not = a: \lnot aRb}.
	\end{center}
